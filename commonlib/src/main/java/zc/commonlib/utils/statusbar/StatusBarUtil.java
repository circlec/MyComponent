package zc.commonlib.utils.statusbar;
...
public class StatusBarUtil {
    ...
    public static void immersiveTitle(Activity activity, int color, View title_layout) {
        immersive(activity.getWindow(), color, 1f);
        if (Build.VERSION.SDK_INT < 23)
            StatusBarUtil.setPaddingSmart(activity, title_layout);
    }
    ...
    public static void setStatusColor(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            LightStatusBarCompat.setLightStatusBar(window, !isColorDark(color));
        }
...
//        window.setStatusBarColor(Color.TRANSPARENT);
    ...
    /**
     * 减少View的paddingTop,减少的值为状态栏高度 (智能判断，并设置高度)
     */
    public static void setPaddingCut(Context context, View view) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            ViewGroup.LayoutParams lp = view.getLayoutParams();
            if (lp != null && lp.height > 0) {
                lp.height += getStatusBarHeight(context);//增高
